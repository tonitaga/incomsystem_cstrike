...
new amx_damager_block_wall;
new amx_damager_multi_hits_count;
new amx_damager_attacker_flags;
new amx_damager_attacker_color[3];
new Float:amx_damager_attacker_position[2];
new amx_damager_spectators_flags;
new amx_damager_spectators_color[3];
new Float:amx_damager_spectators_position[2];
...
public plugin_cfg()
{
	bind_pcvar_num(create_cvar("amx_damager_block_wall", "0", _, "Show damage through walls^n0 - disabled^n1 - enabled", true, 0.0, true, 1.0), amx_damager_block_wall);
	bind_pcvar_num(create_cvar("amx_damager_multi_hits", "1", _, "Show multi-hits counter if they more than one.^n0 - disabled^n1 - enabled", true, 0.0, true, 1.0), amx_damager_multi_hits_count);

	g_pCvarAttackerFlags = create_cvar("amx_damager_attacker_flags", DEFAULT_ATTACKER_FLAGS, _, "Attacker damager flags");
	UpdateFlagsVar(amx_damager_attacker_flags, DEFAULT_ATTACKER_FLAGS);
	hook_cvar_change(g_pCvarAttackerFlags, "@HandleCvarChanged");

	g_pCvarAttackerColor = create_cvar("amx_damager_attacker_color", DEFAULT_ATTACKER_COLOR, _, "Attacker damager colors");
	UpdateColorVar(amx_damager_attacker_color, g_pCvarAttackerColor, DEFAULT_ATTACKER_COLOR);
	hook_cvar_change(g_pCvarAttackerColor, "@HandleCvarChanged");

	g_pCvarAttackerPosition = create_cvar("amx_damager_attacker_position", DEFAULT_ATTACKER_POSITION, _, "Attacker damager position");
	UpdatePositionVar(amx_damager_attacker_position, g_pCvarAttackerPosition, DEFAULT_ATTACKER_POSITION);
	hook_cvar_change(g_pCvarAttackerPosition, "@HandleCvarChanged");

	g_pCvarSpectatorsFlags = create_cvar("amx_damager_spectators_flags", DEFAULT_SPECTATORS_FLAGS, _, "Spectators damager flags");
	UpdateFlagsVar(amx_damager_spectators_flags, DEFAULT_SPECTATORS_FLAGS);
	hook_cvar_change(g_pCvarSpectatorsFlags, "@HandleCvarChanged");

	g_pCvarSpectatorsColor = create_cvar("amx_damager_spectators_color", DEFAULT_SPECTATORS_COLOR, _, "Spectators damager colors");
	UpdateColorVar(amx_damager_spectators_color, g_pCvarSpectatorsColor, DEFAULT_SPECTATORS_COLOR);
	hook_cvar_change(g_pCvarSpectatorsColor, "@HandleCvarChanged");

	g_pCvarSpectatorsPosition = create_cvar("amx_damager_spectators_position", DEFAULT_SPECTATORS_POSITION, _, "Spectators damager position");
	UpdatePositionVar(amx_damager_spectators_position, g_pCvarSpectatorsPosition, DEFAULT_SPECTATORS_POSITION);
	hook_cvar_change(g_pCvarSpectatorsPosition, "@HandleCvarChanged");

	AutoExecConfig(true, "amx_damager");
}
...
@CBasePlayer_TakeDamage_Post(id, inflictor, attacker, Float:damage, bitsDamageType)
{
	...
	if (amx_damager_block_wall && !IsPlayerInView(attacker, id, bitsDamageType))
		return;
	...
	if (amx_damager_multi_hits_count && g_iTotalHits[attacker] > 1)
	...
	if (!amx_damager_attacker_flags || get_user_flags(attacker) & amx_damager_attacker_flags)
	{
		new color[3];
		new Float:position[2];

		color = amx_damager_attacker_color;
		position = amx_damager_attacker_position;
...
ShowDamageToSpectators(attacker, damage[])
{
	...
	color = amx_damager_spectators_color;
	position = amx_damager_spectators_position;
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		if (amx_damager_spectators_flags && !(get_user_flags(i) & amx_damager_spectators_flags))
			continue;
...
@HandleCvarChanged(pCvar, const oldValue[], const newValue[])
{
	...
	{
		UpdateFlagsVar(amx_damager_attacker_flags, newValue);
	}
	else if (pCvar == g_pCvarAttackerColor)
	{
		UpdateColorVar(amx_damager_attacker_color, pCvar, newValue, DEFAULT_ATTACKER_COLOR);
	}
	else if (pCvar == g_pCvarAttackerPosition)
	{
		UpdatePositionVar(amx_damager_attacker_position, pCvar, newValue, DEFAULT_ATTACKER_POSITION);
	}
	...
	{
		UpdateColorVar(amx_damager_spectators_color, pCvar, newValue, DEFAULT_SPECTATORS_COLOR);
	}
	else if (pCvar == g_pCvarSpectatorsPosition)
	{
		UpdatePositionVar(amx_damager_spectators_position, pCvar, newValue, DEFAULT_SPECTATORS_POSITION);
	}
}
